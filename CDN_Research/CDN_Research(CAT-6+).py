...
def open_excels():
    ...
    sheet_load = source.get_sheet_by_name("CAT-6 & Undefined")
    destination = xlsxwriter.Workbook("CDN_Research(CAT-6+).xlsx")
...
def get_domain(loadsheet, savesheet):
    ...
    for rownum in range(2, 486):  # 복사붙여넣기 용 : loadsheet.max_row+1
        try:
            # 2-1-1. 로그
